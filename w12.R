...
svmLin <- svm(y~., data=dfr, kernel="linear", cost=10, scale=FALSE)
...
svmTuned <- svm(y~., data=dfr, kernel="radial", cost=10, gamma=0.3, scale=FALSE)
summary(svmTuned)
...
svm7v <- svm(IfSigned ~ NoPay + NoCalls + TOA + IfMan + 
  M_LastPaymentToImportDate + NoVisits + D_ContractDateToImportDate, 
  data=casesTrn, kernel="radial", cost=10, scale=TRUE)
...
# system.time({ # 930 sec.
#  set.seed(123)
#  svmTune <- tune(svm, train.x=casesTrn[, .SD, .SDcols=c("NoPay", "NoCalls", "TOA",
#    "IfMan", "M_LastPaymentToImportDate", "NoVisits", 
#    "D_ContractDateToImportDate")], train.y=casesTrn$IfSigned, kernel="radial",
#    tunecontrol=tune.control(cross=10),
#    ranges=list(cost=c(1, 2, 5, 10), gamma=c(0.05, 0.1, 0.5, 1.0)))
# })
...
svm7vTuned <- svm(IfSigned ~ NoPay + NoCalls + TOA + IfMan + 
    M_LastPaymentToImportDate + NoVisits + D_ContractDateToImportDate, 
  data=casesTrn, kernel="radial", cost=10, gamma=0.05, scale=TRUE)
...
frmCl <- as.formula("IfSigned + IfNotSigned ~ NoCalls + NoPay + 
  D_ContractDateToImportDate + IfMan + Age + NoVisits + TOA")
...
# predykcja
nnetFitClPred <- compute(nnetFitCl, casesTstStd[, .SD, .SDcols=c("NoCalls", 
  "NoPay", "D_ContractDateToImportDate", "IfMan", "Age", "NoVisits", "TOA")])
...
frm <- as.formula("IfSigned ~ NoCalls + NoPay + 
  D_ContractDateToImportDate + IfMan + Age + NoVisits + TOA")
...
# Predykcja
nnetFitPred <- compute(nnetFit, casesTstStd[, .SD, .SDcols=c("NoCalls", "NoPay", 
  "D_ContractDateToImportDate", "IfMan", "Age", "NoVisits", "TOA")])
...
nnetFitPred <- compute(nnetFit, casesTstStd[, .SD, 
  .SDcols=c("NoCalls", "Bailiff", "D_ContractDateToImportDate", "NoPay", 
  "Age", "TOA", "PrcVisitsCl")])
...
# Uwaga: Kolejne warstwy oraz połączenia pmiędzy nimi sprawiają, że sieć 
...
# Prognoza
xPred <- nnKeras %>% predict_classes(xTst)
xPredB <- nnKerasB %>% predict_classes(xTst)

# Zgodność prognoz pomiędzy metodami
(confModMat <- table(xPred, xPredB))
(accMod <- sum(diag(confModMat))/sum(confModMat))

# Dokładność
(confMat <- table(truth=casesTstStd$IfSigned, predict=xPred))
(accNNetFitK <- sum(diag(confMat))/sum(confMat))  

(confMat <- table(truth=casesTstStd$IfSigned, predict=xPredB))
